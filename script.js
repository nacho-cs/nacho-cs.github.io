...
async function getEloLeaderboard() {
  ...
  res.forEach(row => {
    ...
    tr.innerHTML = `<td> ${
      row["elo_rank"]
    } </td> <td> <a href="./players.html?q=${row.nickname}" class="link"> ${
      row.nickname
    } </a> </td> <td style="color: ${colorElo(
      row["elo_rate"]
...
async function getRunLeaderboard() {
  ...
  res.forEach(row => {
    ...
    tr.innerHTML = `<td> ${
      row["final_time_rank"]
    } </td> <td> <a href="./players.html?q=${row.user.nickname}" class="link"> ${
      row.user.nickname
    } </a> </td> <td> ${new Date(row["final_time"])
      .toISOString()
