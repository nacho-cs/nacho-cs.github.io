...
async function getStats() {
  ...
  )}% </td> <td style="color: ${eloColor}" title="${addRank(stats["elo_rate"])}"> ${stats["elo_rate"]} </td> <td> ${
...
async function getLb() {
  ...
  for (let i = 0; i < lb.length; i++) {
    ...
    tr.innerHTML = `<td> ${
      ...
      lb[i].nickname
    } </a> </td> <td style="color: ${eloColor};" title="${addRank(elo)}"> ${elo} </td>`;
    tr.classList.add("tr");
