...
async function getStats() {
  ...
  } </td> <td> ${stats["highest_winstreak"]} </td> <td> ${new Date(
...
async function getLb() {
  ...
  for (let i = 0; i < lb.length; i++) {
    ...
    tr.innerHTML = `<td> ${
      ...
      lb[i].nickname
    } </td> <td style="color: ${eloColor};"> ${elo} </td>`;
    tr.classList.add("tr");
...
async function getMatches() {
  ...
  matches.forEach((match) => {
    ...
    for (let i = 0; i < match.members.length; i++) {
      players.push(match.members[i].nickname);
    }
    // add evrything to the website
    const tr = document.createElement("tr");
    tr.innerHTML = `<td title="${players.join(", ")}"> ${
      players.length
    } Players </td> <td> ${
      match["match_seed"]
